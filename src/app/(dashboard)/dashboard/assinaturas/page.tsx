...
// Mapa de DDD -> UF, usado como último recurso quando o dono não preencheu a
// cidade nem o estado (a cidade é opcional no cadastro). Assim ainda dá para
// saber a regiao pelo numero do WhatsApp.
...
// Deduz a UF a partir do WhatsApp (pega o DDD, ignorando o 55 do pais).
function ufFromWhatsapp(w: string | null): string | null {
  const d = (w || '').replace(/\D/g, '')
  const local = d.length > 11 && d.startsWith('55') ? d.slice(2) : d
  return DDD_UF[local.slice(0, 2)] ?? null
}
